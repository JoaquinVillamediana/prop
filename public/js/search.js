...
function updateProps(data) {
    ...
    if (data.aProperties.length > 0) {
        data.aProperties.forEach(element => {
            ...
            item = item + '</div><a class="carousel-control-prev" href="#carouselPropControls-' + element.id + '" role="button"data-slide="prev"><span class="carousel-control-prev-icon" aria-hidden="true"></span><span class="sr-only">Previous</span></a><a class="carousel-control-next" href="#carouselPropControls-' + element.id + '"role="button" data-slide="next"><span class="carousel-control-next-icon"aria-hidden="true"></span><span class="sr-only">Next</span></a></div></div><div class="col-xl-6 col-12"><a href="http://192.168.0.200:8080/propietie/' + element.id + '"><div class="card-block "><h3 class="card-title mt-2 "> ' + element.name + ' </h3><h3 class="price"><span class="currency"><span class="currency-symbol">$</span></span> ' + formatNumber.new(element.price) + '</h3><p class="card-text description">' + element.description + '</p><p class="characteristics"><span id="rooms" class="characteristic" data-toggle="tooltip" data-placement="top"title="' + element.rooms + ' Ambientes"><i class="fas fa-home"></i>Cuartos <span class="quantity">' + element.rooms + '</span></span><span id="bathrooms" class="characteristic" data-toggle="tooltipdata-placement="top"title="' + element.bathrooms + ' Baño"><i class="fas fa-toilet"></i>Baños <span class="quantity">' + element.bathrooms + '</span></span><span id="bedrooms" class="characteristic" data-toggle="tooltipdata-placement="top"title="' + element.bedrooms + ' Dormitorio"><i class="fas fa-bed"></i>Dormitorios <span class="quantity">' + element.bedrooms + '</span></span> <span id="size" class="characteristic" data-toggle="tooltip" data-placement="top"title="' + element.size + ' m2"><i class="fas fa-ruler-combined"></i>Tamaño <span class="quantity">' + element.size + 'm<sup>2</sup></span></span> <span id="antiquity" class="characteristic" data-toggle="tooltip" data-placement="top"title="' + element.years + ' años"><i class="fas fa-clock"></i>Antiguedad <span class="quantity">' + element.years + ' años</span></span>  </p><a href="http://192.168.0.200:8080/propietie/' + element.id + '" id="btncontacto" class="btn btn-contact d-block">Ver más</a></div></a></div></div></div>';
